...
def read_csv():
    content = ''
    with open('maoyan.csv','r',encoding='utf_8_sig',newline='') as file_test:
...
def word_cloud(content):
    ...
    wordcloud.render(".\picture\c_wordcloud.html")
...
def sex_distribution(gender):
    ...
    pie.render(".\picture\sex_pie.html")
...
def city_distribution(cityName):
    ...
    # sort_dict = sorted(zip(city_dict.values(),city_dict.keys()),reverse=True)
    city_name = []
    city_num = []
    ...
    bar.render(".\picture\city_bar.html")
...
def time_num_visualization(time):
    ...
    line.render(".\picture\c_num_line.html")


# 评论者猫眼等级、评分可视化
def level_score_visualization(userLevel, score):
    from pyecharts import Pie
    userLevel.remove('猫眼等级')
    ...
    pie01.render(".\picture\level_pie.html")
    ...
    pie02.render(".\picture\score_pie.html")
...
# make_wordcloud(jiebaclearText(content))
# pyecharts词云
# word_cloud(content)
# # 2 性别分布
